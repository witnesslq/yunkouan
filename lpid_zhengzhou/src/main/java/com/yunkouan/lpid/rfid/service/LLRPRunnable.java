...
import com.yunkouan.lpid.biz.screenscompare.service.impl.ScreensCompareHistoryServiceImpl;
import com.yunkouan.lpid.commons.util.Constant;


public class LLRPRunnable implements LLRPEndpoint,Runnable {
	private ExecutorService service = Executors.newCachedThreadPool();
	private String ip;
    private LLRPConnection connection;

    private static org.slf4j.Logger logger  = org.slf4j.LoggerFactory.getLogger(LLRPRunnable.class); 
    ...
    private void connect(String ip) {
        ...
        } catch (LLRPConnectionAttemptFailedException | RuntimeIOException e1) {
        	ThreadUtil.sleep(1000);
            logger.error("天线发卡器连接失败,重新连接中..." + ip);
//            connect(ip);
        }
    }
    ...
    private ADD_ROSPEC buildROSpecFromFile() {
        ...
        URL url = this.getClass().getClassLoader().getResource("/com/yunkouan/lpid/rfid/service");
//        URL url = this.getClass().getResource(".");
		if(url == null){
			logger.error("url is null_2222");
			return null;
		}
...
//        URL url = this.getClass().getResource(".");
        ...
        try {
            ...
            new File(currentPath + "/SET_READER_CONFIG.xml"));
...
//		showRunningPackageModels(epc);
        ...
        service.execute(new PushPaggageMessage(epc));
    }
    
    
    
    /*private void showRunningPackageModels(String epc) {
    	while(this.screensCompareHistoryService == null) {
			this.screensCompareHistoryService = SpringApplicationContext.getBean(ScreensCompareHistoryServiceImpl.class);
			logger.debug("screensCompareHistoryService == null");
			ThreadUtil.sleep(500);
		}
		RunningPackageModel runningPackageModel = screensCompareHistoryService.getRunningPackageModelByEPC("300833B2DDD9014000000001");
...
//        logger.debug("Received " + message.getName() + " message asychronously");
    ...
    public static void main(String[] args) {
        ...
        if  (args.length < 1) {
        	logger.debug("Must pass reader hostname or IP as agument 1");
        }

        // Only show root events from the base logger
        Logger.getRootLogger().setLevel(Level.ERROR);
        LLRPRunnable example = new LLRPRunnable();
//        logger.setLevel(Level.INFO);

//        example.connect(args[0]);
        example.connect("192.168.3.61");
...
//        System.exit(0);
	...
	@Override
	public void run() {
		while(SpringApplicationContext.springBeanFactory == null) {
			logger.debug("SpringApplicationContext.springBeanFactory == null");
			ThreadUtil.sleep(500);
		}
...
class PushPaggageMessage implements Runnable {
	private ScreensCompareHistoryService screensCompareHistoryService;
	private static org.slf4j.Logger logger  = org.slf4j.LoggerFactory.getLogger(PushPaggageMessage.class); 
	private String epc;
	PushPaggageMessage(String epc){
		this.epc = epc;
	}
	@Override
	public void run() {
		showRunningPackageModels();
	}
	private void showRunningPackageModels() {
    	while(screensCompareHistoryService == null) {
			screensCompareHistoryService = SpringApplicationContext.getBean(ScreensCompareHistoryServiceImpl.class);
			logger.debug("screensCompareHistoryService == null");
			ThreadUtil.sleep(500);
		}
		RunningPackageModel runningPackageModel = screensCompareHistoryService.getRunningPackageModelByEPC(this.epc);
		List<XrayImageModel> xrayImmageModels = runningPackageModel.getXrayImages();
		Iterator<XrayImageModel> iteratorImage = xrayImmageModels.iterator();
		while(iteratorImage.hasNext()) {
			XrayImageModel xrayImageModel = iteratorImage.next();
    	...
    	logger.info("三视角X光机推送的json数据:{}" , result);
	}
}
